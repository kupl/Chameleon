...
int Search::LaunchProgram(const vector<value_t>& inputs) {
  ...
  exitcode = system(program_.c_str())/256;
  return exitcode;
}
...
void Search::RunProgram(const vector<value_t>& inputs, SymbolicExecution* ex) {
  ...
  int signalnum;
  // Run the program.
  signalnum = LaunchProgram(inputs) - 128;
  ...
  // Collect bug-triggering inputs by monitoring the system signals (e.g., SIGSEGV).
  if (0 < signalnum && signalnum < 65){
    string base_name = "_buginput_"; 
    ...
    string bug_file = snum+base_name+timestr;

    ifstream source("input", ios::binary);
    ofstream dest(bug_file, ios::binary);
...
bool Search::UpdateCoverage(const SymbolicExecution& ex,
  ...
  if(num_iters_ == 1 || num_iters_ == 4000 || num_iters_ == 8000){
...
bool ParameterizedSearch::DoSearch(SymbolicExecution& ex)
{
  ...
  while (!candidates.empty()) {
    ...
    bool covered_new = UpdateCoverage(cur_ex);
